...
require 'EmailSender2.php'; // 引入EmailSender.php文件


class TicketModel extends BasicModel
{
	protected $_filter = [
		'ticket_no', 'title', 'ticket_model_id', 'status_id', 'photo', 'priority', 'look', 'group_id', 'dispose_time', 'dispose_id',
     ...
     * @param array $ticketDetails 工单详情
     * @param string $user_id 用户ID
     * @param int $ticket_model_id 工单模型ID
     * @return string 工单编号
     */
	public function getTicketNo($company_id = 0, $ticketDetails = [], $user_id = '', $ticket_model_id = 0)
{
    $redis = new Redis();
    $date = date('Ymd'); // 获取当前日期，格式为YYYYMMDD
    $cache_key = 'ticketNo_' . $date; // 防重集合键

    // 使用不同的计数器键基于不同的 ticket_model_id
    if ($ticket_model_id == 7 || $ticket_model_id == 8) {
        // 对于模板ID为7或8的情况，直接在 createTicketNo 中处理特定逻辑
        $ticket_no = $this->createTicketNo($company_id, "", 0, $ticket_model_id);
    } elseif ($ticket_model_id == 5) {
        $cache_key_counter = 'ticketNo_counter_' . $date; // 计数器键
        // 如果Redis中没有该日期的计数器，则初始化为0
        if (!$redis->exists($cache_key_counter)) {
            $redis->set($cache_key_counter, 0);
        }

        $count = $redis->incr($cache_key_counter); // 递增计数器
        $count = str_pad($count, 2, '0', STR_PAD_LEFT); // 将计数器转换为两位数的字符串

        // 查找 ticket_form_id 为 9 的期刊类型字段的值
        $journalTypeValue = '';
        foreach ($ticketDetails as $detail) {
            if ($detail['ticket_form_id'] == 9) {
                $journalTypeValue = $detail['ticket_form_content'];
                break;
            }
        }

        if (!$journalTypeValue) {
            $journalTypeValue = "";  // Use an empty string for clarity
        }

        $ticket_no = $this->createTicketNo($company_id, $journalTypeValue, $count, $ticket_model_id);
    }else{
		$ticket_no = $this->createTicketNo($company_id, "", 0, $ticket_model_id);
	}
    ...
    if ($redis->sIsMember($cache_key, $ticket_no)) {
        return $this->getTicketNo($company_id, $ticketDetails, $user_id, $ticket_model_id);
    } else {
        // 防重 - 将已生成的工单编号存入集合
        $redis->sAdd($cache_key, $ticket_no);

        // 防重 - 设置工单编号集合有效期为1天
        $redis->expire($cache_key, 86400); // 86400秒 = 24小时
    }
     ...
     * 生成工单编号
     * @param int $company_id 公司ID
     * @param string $journalType 日志类型
     * @param int $count 计数器值
     * @param int $ticket_model_id 工单模型ID
     * @return string 工单编号
     */
	private function createTicketNo($company_id = '', $journalType = '', $count = 0, $ticket_model_id = 0)
{
    $dateStr = date('Ymd', time()); // 获取当前日期
    $redis = new Redis(); // 假设已经建立连接

    // 为特定模板ID设置独立计数器
    if ($ticket_model_id == 7 || $ticket_model_id == 8) {
        $typePrefix = $ticket_model_id == 7 ? "JournalSuggestion" : "SuggestTitle";
        $consultKey = $typePrefix . '_count_' . $dateStr; // 独立计数器

        if (!$redis->exists($consultKey)) {
            $redis->set($consultKey, 0);
        }
        $count = $redis->incr($consultKey);
        $count = str_pad($count, 2, '0', STR_PAD_LEFT);

        return $typePrefix . "_" . $count;
    } elseif ($ticket_model_id == 5) {
        // 默认情况下使用传入的计数器和类型
        return $journalType . '_' . $dateStr . '_' . $count;
    }else{
		$typePrefix =  "System";
        $consultKey = $typePrefix . '_count_' . $dateStr; // 独立计数器
        ...
        return $typePrefix .  $dateStr. "_" . $count;
	}
}
    ...
    // {
    //     if ($journalType) {
    //         return $journalType . '_' . date('Ymd', time()) . '_' . $count;
    //     } else {
    //         $dateStr = date('Ymd', time()); // 获取当前日期
    //         $redis = new Redis(); // 假设已经建立连接
    //         $consultKey = 'Consult_count_' . $dateStr; // 为"Consult"类型设立独立计数器
    ...
    //         $consultCount = $redis->incr($consultKey); // 递增计数器
    //         $consultCount = str_pad($consultCount, 2, '0', STR_PAD_LEFT); // 格式化计数器

    //         switch ($ticket_model_id) {
    //             case 7:
    //                 return "JournalSuggestion_" . $consultCount;
    //             case 8:
    //                 return "SuggestTitle_" . $consultCount;
    //             default:
    //                 return "Consult_" . $consultCount;
    //         }
    //     }
	// }
	...
	public function getCreateTicketForm($template = [], $where = [], $company = [], $member_type = 1)
	{
		...
		if ($member_type == 1) {
			...
			$createTicketFormData['groups'] = M('group')->where(['company_id' => $company['company_id'], 'closed' => 0, 'ticket_auth' => 10])
				...
				->order('orderby asc')
				->select();
			// // 使用正则表达式过滤出包含“编辑”和“对接”的部门
			// $allowedGroups = [];
			// foreach ($createTicketFormData['groups'] as $group) {
			// 	if (preg_match('/(编辑|对接|写作|投稿|财务)/', $group['group_name'])) {
			// 		$allowedGroups[] = $group;
			// 	}
			// }

			// // 更新部门列表为只包含允许的部门
			// $createTicketFormData['groups'] = $allowedGroups;
			...
			foreach ($createTicketFormData['members'] as $mk => $mv) {
				...
				// 				// 使用正则表达式检查名字是否符合特定的角色模式
				// $isAllowedRole = preg_match('/(出纳|编辑|对接|投稿|财务)$/', $mv['name']);

				// //                移除不在权限部门中的用户
				// if (!$isAllowedRole || ($result && empty(array_diff($group_id, $result)))) {
    ...
    */
	public function CreateTicketByUser($company_id = 0, $member_id = 0, $template = [], $first_status_id = 0, $ticket_from = '', $source = 'page', $importData = [])
	{
		...
		error_log("Message to be sent333333.}");
    ...
    */
    ...
    // 判断图片数组
    $photos = $_POST['photo'] ?? [];
    if ($ticket['status_id'] == 9 && count($photos) < 2) {
        return ['status' => 0, 'msg' => L('PLEASE_UPLOAD_TWO_PHOTOS')];
    }
    ...
    */
	public function getTicketDetailData(&$ticket, $company_id, $now_member, $ticket_status, $source)
	{
		...
		if (
			$ticket['status_id'] == $ticket['end_status_id'] && $ticket['delete'] == 2  && $ticketConfig['end_operate'] == 10 &&
    ...
    */
	public function ticketReplyByUser($company_id = 0, $now_member_id = 0, $end_status_id = 0)
	{
		...
		if ($reply_id = M('TicketReply')->add($data)) {
			...
			error_log("接收人: " . $detail['dispose_id']);

			// 发送邮件通知
			$emailSender = new EmailSender2();
			// 假设 $detail['dispose_id'] 是处理人的ID
			$recipient_id = $detail['dispose_id'];

			// 查询处理人的邮箱地址
			$recipient = M('member')->where(['member_id' => $recipient_id])->field('email, last_active_time')->find();
			$recipient_email = $recipient['email'];
			$last_active_time = $recipient['last_active_time'];
			$effective = time() - 5 * 60;
			error_log("各种时间: $last_active_time,$effective");
			// 确保邮箱地址不为空
			// if (!empty($recipient_email) && $last_active_time < $effective) {
			// 	// 现在 $recipient_email 包含了处理人的邮箱地址
			// 	// 接下来，您可以使用这个邮箱地址来发送邮件通知

			// 	// 抄送人数组
			// 	$ccAddresses = [];

			// 	// 查询抄送人信息，这里假设抄送人信息是通过CcTicket模型来查询的
			// 	$cc = D('CcTicket')->where(['company_id' => $company_id, 'ticket_id' => $data['ticket_id']])->field('member_id')->select();

			// 	// 将抄送人ID转换成邮箱地址，这里需要您根据实际情况来获取邮箱
			// 	foreach ($cc as $ccMember) {
			// 		$ccMemberEmail = M('member')->where(['member_id' => $ccMember['member_id']])->field('email')->find();
			// 		if (!empty($ccMemberEmail['email'])) {
			// 			$ccAddresses[] = $ccMemberEmail['email'];
			// 		}
			// 	}

			// 	// 发送邮件通知
			// 	$emailSender = new EmailSender2();
			// 	error_log("各种信息: $recipient_email,$ccAddresses");
			// 	try {
			// 		$response = $emailSender->sendMail($recipient_email, '新回复', '你有一个新回复！', $ccAddresses);
			// 		// 其余代码...
			// 	} catch (Exception $e) {
			// 		// 邮件发送失败的处理
			...
			// 	}
			// } else {
			// 	error_log("No email address found for recipient ID: " . $recipient_id);
			// }
	...
	*/
	private function checkCreateTicket($company_id = 0, $user_id = 0, $create_source = 'user', $template = [], $source = 'page', $importData = [])
	{
		...
		$ticket['ticket_no'] = $this->getTicketNo($company_id, $ticket_detail, $user_id, $ticket['ticket_model_id']);
    ...
    */
	...
	public function getStartTime($time_id = '-1')
	{
		$timestamp = [
			...
			self::ONE_MONTH  => strtotime('-1 year')
		];
			...
			if ($this->where($where)->delete()) {
				...
				if (M('ticket_detail')->where($where)->delete()) {
					foreach ($contents as $c) {
						$content = str_replace(
							'?imageMogr2/auto-orient/thumbnail/240x135!/format/png/blur/1x0/quality/100|imageslim',
							'',
				...
				if ($inside_reply_result) {
					foreach ($inside_reply as $ir) {
						$inside_reply_content = str_replace(
							'?imageMogr2/auto-orient/thumbnail/240x135!/format/png/blur/1x0/quality/100|imageslim',
							'',
				...
				$table = [
					'reply_comment', 'cc_ticket', 'ticket_event', 'ticket_deadlines', 'system_message',
...
error_log("更新开始");
	...
	*/
	public function updateTicketStatus($company_id, $ticket, $status_id, $end_status_id, $now_member_id, $type = 1)
	{
		...
		// error_log("哈哈哈哈哈");
		// // 判断图片数组
		// error_log($ticket['status_id']);
		// error_log(count($photo));
		// if ($status_id == 9 && count($photo) < 2) {
		// 	error_log("少图片了");
		// 	return ['errcode' => 1, 'msg' => '请上传定金或全款截图！'];
		// }
	...
	public function urgeTicket($company_id = 0, $now_member_id = 0, $source = 'user')
	{
		...
		if (empty($urgeData['members'])) {
			...
			foreach ($members as $v) {
				//                系统
				D('BuildMessage')->buildSystemMessage(
					'urge_ticket',
					$v['type'],
					$v['member_id'],
					$urgeData['ticket_id'],
			...
			if ($source == 'user') {
				//              新增内部协作记录
				$teamReplyData = [
					'company_id' => $company_id, 'member_id' => $now_member_id, 'ticket_id' => $urgeData['ticket_id'],
					...
					try {
						...
						for ($rowIndex = 1; $rowIndex <= $rows; $rowIndex++) {
							...
							if ($formData) {
								...
								// 用户 - 创建工单
								$result = D('Ticket')->CreateTicketByUser(
									$company_id,
									$member_id,
									$template,
									$first_status_id,
