...
require 'EmailSender.php'; // 引入EmailSender.php文件
...
class BuildMessageModel extends BasicModel
{
	...
	private $host = 'smtp.em.dingtalk.com';  // SMTP 服务器地址
	private $username = 'mayao@zdpress.top';  // SMTP 用户名
	private $password = 'h7GfC5fsKi7MM6la';  // SMTP 密码
	private $port = 465;  // SMTP 端口
	private $secure = 'ssl';  // 加密方式
    ...
    */
	...
	{
		error_log("？$msg_type");
error_log("This is a test error", 3, "/www/server/nginx/ticker-error.log");
		//    	子工单
		if ($isSubTicket) {
		...
		// 		$member = M('member')->where(['member_id' => $member_id])->field('email')->find();
		// $recipient_email = $member['email'];

		// msg_type
		//reply 工单回复，team_reply 内部协作，update_status 更新状态，restart_ticket 重启工单，edit_ticket 修改工单


		$recipient = M('member')->where(['member_id' => $member_id])->field('email, login_status')->find();
		$recipient_email = $recipient['email'];
		$login_status = $recipient['login_status'];
		// $ticket = M('ticket')->where(['ticket_id' => $ticket_id])->field('ticket_id,ticket_no,company_id,title')->find();
		$ticket = M('ticket')->where(['ticket_id' => $ticket_id])->field('ticket_id,ticket_no,company_id,title,member_id')->find();

		// 获取工单的发布人信息，假设工单信息中的member_id是发布人ID
		$publisher = M('member')->where(['member_id' => $ticket['member_id']])->field('email, login_status')->find();

		// 检查发布人的邮箱地址和登录状态
		if (!empty($publisher['email'])) {
			$publisherEmail = $publisher['email'];
			$subject = "关于销售支持的工单 {$ticket['ticket_no']} 的通知";
			$body = "您好，您的工单编号 {$ticket['ticket_no']} 的工单有新进展。<br><br>标题：{$title}<br><br>请查看以下链接以获取更多信息：<a href=\"{$url}\" target=\"_blank\">查看工单</a>";

			// 使用邮件发送类发送邮件
			try {
				$emailSender = new EmailSender();
				$response = $emailSender->sendMail($publisherEmail, $subject, $body);
				// 可能还需要一些逻辑来处理邮件发送的结果
			} catch (Exception $e) {
				error_log("Failed to send email to publisher: " . $e->getMessage());
			}
		}

		// 确保邮箱地址不为空
		if (!empty($recipient_email) && $login_status  == 0) {
		//if ( $login_status  == 0) {
			$url = "http://47.122.65.170" . $url;
			switch ($msg_type) {
				case 'reply':
					// 工单回复
					$subject = "{$ticket['ticket_no']}-{$title}有新回复：";
					$body = "您好，您收到关于工单编号 {$ticket['ticket_no']} 的新回复。<br><br>标题：{$title}<br>请查看以下链接以获取更多信息：<a href=\"<a href=\"{$url}\" target=\"_blank\">查看工单</a>\" target=\"_blank\">查看工单</a>";
					break;
				case 'team_reply':
					// 内部协作回复
					$subject = "内部协作回复: {$title}";
					$body = "您好，团队成员对工单编号 {$ticket['ticket_no']} 发送了内部协作回复。<br><br>标题：{$title}<br><br>详情请访问：<a href=\"{$url}\" target=\"_blank\">查看工单</a>";
					break;
				case 'update_status':
					// 更新状态
					$subject = "工单状态更新: {$title}";
					$body = "您好，工单编号 {$ticket['ticket_no']} 的状态已更新。<br><br>标题：{$title}<br><br>请查看以下链接以获取更多信息：<a href=\"{$url}\" target=\"_blank\">查看工单</a>";
					break;
				case 'restart_ticket':
					// 重启工单
					$subject = "工单重启通知: {$title}";
					$body = "您好，工单编号 {$ticket['ticket_no']} 已被重启。<br><br>标题：{$title}<br>请查看以下链接以继续处理：<a href=\"{$url}\" target=\"_blank\">查看工单</a>";
					break;
				case 'edit_ticket':
					// 修改工单
					$subject = "工单信息更新: {$title}";
					$body = "您好，工单编号 {$ticket['ticket_no']} 的信息已更新。<br><br>标题：{$title}<br><br>详情请访问：<a href=\"{$url}\" target=\"_blank\">查看工单</a>";
					break;
				case 'dispose_reply_ticket':
				$subject = "{$ticket['ticket_no']}-{$title}有新回复：";
					$body = "您好，工单编号 {$ticket['ticket_no']} 已被回复。<br><br>标题：{$title}<br><br>详情请访问：<a href=\"{$url}\" target=\"_blank\">查看工单</a>";
					break;
				default:
					$subject = "新通知: {$title}";
					$body = "您好，您有一个新的工单通知。<br><br>工单编号：{$ticket['ticket_no']}<br>标题：{$title}<br>请查看以下链接：<a href=\"{$url}\" target=\"_blank\">查看工单</a>";
			}

			$emailSender = new EmailSender();

			// 抄送人
			$ccAddresses = $this->getCCMemberIdsEmail($ticket_id);

			try {
				$response = $emailSender->sendMail($recipient_email, $subject, $body, $ccAddresses);
				// 其余代码...
			} catch (Exception $e) {
				// 邮件发送失败的处理
...
*/
	...
	private function getCCMemberIdsEmail($ticket_id)
	{
		// 查询抄送人的邮箱地址
		// 假设 'cc_ticket' 表用于关联工单和抄送人
		// 假设 'member' 表包含成员的邮箱地址
		$cc_member_ids = M('cc_ticket')
			->where(['ticket_id' => $ticket_id])
			->getField('member_id', true); // 获取所有抄送人的ID数组

		$cc_emails = [];

		// 遍历抄送人的用户ID数组
		foreach ($cc_member_ids as $member_id) {
			// 查询每个抄送人的邮箱地址
			$member = M('member')->where(['member_id' => $member_id])->field('email')->find();

			// 检查查询结果并添加到邮箱地址数组中
			if ($member && !empty($member['email'])) {
				$cc_emails[] = $member['email'];
			}
		}
		return $cc_emails;
	}
