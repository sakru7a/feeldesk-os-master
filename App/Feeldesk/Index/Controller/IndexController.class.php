...
class IndexController extends BasicController
{
	public function index()
	{
		...
		{
			$redirect_url = $url ? '/u-home-base?redirect_url='.$url : '/u-home-base';
		}
		else
		{
			$redirect_url = $url ?: '/u-home-base';
		}

		$from_type = I('get.from_type') ? decrypt(I('get.from_type'),'FROMTYPE') : '';

        $systemAuth = M('company')->where(['company_id'=>$this->_company_id])->field('ticket_auth')->find();
...
//		 我处理的工单
		$ticketData['ticket'] = M('ticket')->where(['company_id'=>$this->_company_id,'dispose_id'=>$this->member_id,'end_time'=>0,'delete'=>2])
			->field('ticket_id,status_id,priority,member_id,title,ticket_no')
			->select();
...
//		 所有用户及会员
		...
		$notices = M('Notices')->select(); 

			// 向视图分配数据
		$this->assign('noticeData', $notices);
    ...
    {
        $redirect_url = $redirect_url ? U($redirect_url) : '/u-home-workbench';
